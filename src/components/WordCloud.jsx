...
export default function WordCloud({ results, mode = "overlap" }) {
  ...
  const words = useMemo(() => {
    ...
    return top100.map(a => {
      ...
      let color = C.blueLight;
      if (!exclusive) {
        const ratio = a.worksInA / (score || 1);
        color = ratio > 0.6 ? C.blueLight : ratio < 0.4 ? C.amberLight : C.green;
      }
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "20px 24px", marginBottom: 24 }}>
      <div style={{ fontSize: 10, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 4 }}>
        {exclusive
          ? "Top 100 candidate authors (published in Set A, not yet in Set B)"
          : "Top 100 overlapping authors"}
      </div>
      <div style={{ fontSize: 11, color: C.textMuted, marginBottom: 16 }}>
        {exclusive ? (
          <>Size = Set A article count</>
        ) : (
          <>
            Size = overlap score · <span style={{ color: C.blueLight }}>■</span> Set A dominant ·{" "}
            <span style={{ color: C.green }}>■</span> Balanced ·{" "}
      ...
      <div style={{
        ...
        {words.map(w => (
          <a key={w.id} href={w.id} target="_blank" rel="noreferrer" style={{
            ...
            title={exclusive
              ? `${w.name} · ${w.score} Set A article${w.score === 1 ? "" : "s"}`
              : `${w.name} · overlap score: ${w.score}`}
          >
            {w.name}
          </a>
        ))}
